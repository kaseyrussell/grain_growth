...
def voronoi(n, periodic=False, include_nodes=False):
    ...
    if periodic:
        ...
        for i,shiftx in enumerate([-0.5, -1.5, 0.5]):
            for j,shifty in enumerate([-0.5, -1.5, 0.5]):
    ...
    if include_nodes:
        ...
        for d in doubles:
            ...
            for w in wrappers:
                ...
                if (abs(d['point_out'][0]+1.0 - w['point_in'][0])<1.0e-6 or
                    abs(d['point_out'][1]+1.0 - w['point_in'][1])<1.0e-6):

                    """ Does it share both verticies? """
                    if (abs(d['point_in'][0]+1.0 - w['point_out'][0])<1.0e-6 or
                        abs(d['point_in'][1]+1.0 - w['point_out'][1])<1.0e-6):
