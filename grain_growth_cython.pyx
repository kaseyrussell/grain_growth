...
""" Implementation of boundary tracking method from Bronsard & Wetton
    ...
    I now build this using pyximport using the following code before import:
...
cdef class Boundary(object):
    ...
    cdef int initialize_iteration_buffer(self) except *:
        """ Set self.b_prev_timestep to the current value. """
        self.b_prev_timestep = self.b.copy()

    cdef int reset_timestep(self) except *:
        """ Reset self.b to its value at the beginning of the current timestep. """
        self.b = self.b_prev_timestep.copy()
    ...
    cpdef int implicit_step_junction_iteration(self, double timestep) except *:
        ...
        cdef:
            ...
            #np.ndarray[dtype=np.float64_t, ndim=2] b_prev_timestep = np.empty((2,lenb), dtype=np.float64)
        ...
        #b_prev_timestep = self.b_prev_timestep.copy().transpose()
        #b_solve = dgtsvx( offdiagdn, diag, offdiagup, b_prev_timestep ) # fails immediately...
        #self.b = b_solve
        #self.b = tridiagonal_solve( offdiagdn, diag, offdiagup, b_prev_timestep.transpose() ) # fails at time 1.8
        
        M      = np.diag(diag) + np.diag(offdiagup, k=1) + np.diag(offdiagdn, k=-1)
        b_solve = np.linalg.solve( M, self.b_prev_timestep )
        self.b = b_solve
        ##b_prev_timestep = self.b_prev_timestep.copy()
        ##b_solve = dgesv( M, b_prev_timestep.transpose() )
        ##self.b = b_solve.transpose()
        #assert np.shape(b_solve.transpose()) == np.shape(self.b)
    ...
    def curve_thy_neighbors(self, Node node):
        ...
        subs_boundary, subs_endindx, subs_end = self.get_subsequent_boundary(node)
         ...
         # TODO: correct the vectors v0, vprev, and vsubs for periodic b.c.
         #        and alter rotation algorithm accordingly.
    ...
    cpdef get_head_location(self, np.ndarray[dtype=np.float64_t, ndim=1] loc):
        """ Puts the location of the head node (as viewed by the boundary)
        into the given 2-element numpy array,
        shifting it by node._domain_width if it exceeds the periodic b.c. """
        cdef:
            double x,y,x0,y0
            Py_ssize_t indx
    ...
    cpdef get_tail_location(self, np.ndarray[dtype=np.float64_t, ndim=1] loc):
        """ Puts the location of the tail node (as viewed by the boundary)
        into the given 2-element numpy array,
        shifting it by node._domain_width if necessary within periodic b.c. """
        cdef:
            double x,y,x0,y0
    ...
    cpdef get_node_location(self, Node node, np.ndarray[dtype=np.float64_t, ndim=1] xy):
        """ Puts the location of the given node (as viewed by the boundary)
        into the given 2-element numpy array,
        shifting it by node._domain_width if it exceeds the periodic b.c. """
        cdef:
            Boundary gb
        ...
        if node == self.head:
            ...
            raise ValueError, "The given Node object is not connected to this boundary."
    ...
    cdef int check_for_deletion(self):
        """ Check if the boundary will be deleted at the end of the current timestep. """
        cdef:
            Py_ssize_t lenb = len(self.b)
            np.ndarray[np.float64_t, ndim=2] b = self.b
            np.ndarray normd1x2 = np.zeros(lenb-2, dtype=np.float64)
            Py_ssize_t i, stop
            double min_velocity = 0.0
            double D1x, D1y, mu, norm2
            double h = self._h
        


        if self._to_be_deleted: return 1
    ...
    cpdef check_spacing(self, bool nodeless=False):
        ...
        cdef:
            ...
            Py_ssize_t i, stop
            double min_velocity = 0.0
            double D1x, D1y, nu, mu, norm2
            double h = self._h

        if self._to_be_deleted: return False
    ...
    cdef double get_speed_near_node(self, Node node) except *:
        """ Finds the norm of the second derivative of the point 
        ...
        if node == self.head:
            ...
            raise ValueError, "Node object not attached to Boundary."
        ...
        D1norm2 = D1x*D1x + D1y*D1y
        return abs(D2x/D1norm2) + abs(D2y/D1norm2)
...
cdef class Node(object):
    ...
    cdef np.ndarray position_prev_timestep
    cdef int _type
    cdef str _id
    cdef Boundary b
    ...
    def __init__(self, int node_type, double period=1.0e15):
        ...
        self.position_prev_timestep = np.array([0.0, 0.0], dtype=np.float64)
        self.boundaries = []
        self.nodes = []
        self.ends = []
        self.endindx = []
    ...
    cdef int initialize_iteration_buffer(self) except *:
        """ Set self.position_prev_timestep to the current value. """
        self.position_prev_timestep = self.position.copy()

    cdef int reset_timestep(self) except *:
        """ Reset self.position to its value at the beginning of the current timestep. """
        self.position = self.position_prev_timestep.copy()
    ...
    def add_boundaries(self, b1, b2, b3):
        ...
        cdef:
            ...
            np.ndarray[dtype=np.float64_t, ndim=1] p3  = np.empty((2))

        for b in [b1,b2,b3]:
            bndry, indx, end = self.parse_boundary(b)
            #bndry.check_spacing(True)
            self.boundaries.append(bndry)
            self.endindx.append(indx)      # TODO: update endindx appropriately as boundaries are coarsened or densified
            self.ends.append(end)

        if not self.fits_inside_triangle():
            print 'In Node.add_boundaries, adjusting boundaries for node {0} at {1}'.format(self.get_id().split('-')[0], self.position)
            if verbose:
    ...
    def overwrite_extrapolation_points(self):
        """ Store the node location in the extrapolation points of each
        boundary (i.e. the first or last (x,y) point in the boundary array),
        adjusting the Node location appropriately for each boundary to account 
        for periodic boundary conditions. """
        cdef Boundary   b
        cdef Py_ssize_t i
        for b in self.boundaries:
            if self == b.head:
                b.get_head_location(b.b[0])
            else:
                b.get_tail_location(b.b[len(b.b)-1])
    ...
    cdef void get_closest_interior_point(self, Py_ssize_t indx, np.ndarray[dtype=np.float64_t, ndim=1] closest_point) except *:
        """ Return the point on self.boundary[indx] that is closest to this node
        (i.e. point [1] or point [-2] of the boundary) and shift it by 
        self._domain_width as appropriate for the periodic b.c. """
        cdef:
            Boundary gb
    ...
    @cython.boundscheck(False)
    @cython.wraparound(False)
    cpdef get_triangle(self,
        np.ndarray[dtype=np.float64_t, ndim=1] p0,
    ...
    cdef Boundary get_fastest_moving_boundary(self):
        """ Return boundary that has the highest
        second-derivative adjacent to this Node """
        cdef Boundary b
        speed = []
        for b in self.boundaries:
            speed.append( b.get_speed_near_node(self) )
        return self.boundaries[np.argmin(speed)]


    cdef void fix_triangle(self) except *:
        """ Densify fastest-moving boundary. """
        cdef Boundary bndry
        bndry = self.get_fastest_moving_boundary()
        bndry.reset_timestep()
        bndry.densify()
        bndry.initialize_iteration_buffer()
    ...
    cpdef int update_position( self, bool verbose=False, bool check_angles=False ) except *:
        ...
        if self._type == domain_boundary:
            ...
            if fits_inside == false:
                ...
                bndry = self.get_obtuse_boundary()
                if False:
                    # curve_thy_neighbors expects the 'target node' location
                    # to be stored in the extrapolation storage points
    ...
    cdef int fits_inside_triangle(self) except *:
        ...
        cdef:
            ...
            np.ndarray[dtype=np.float64_t, ndim=1] p2  = np.empty((2))
            np.ndarray[dtype=np.float64_t, ndim=1] p3  = np.empty((2))
            np.ndarray[dtype=np.float64_t, ndim=1] v13 = np.empty((2))
            np.ndarray[dtype=np.float64_t, ndim=1] v21 = np.empty((2))
            np.ndarray[dtype=np.float64_t, ndim=1] v23 = np.empty((2))
            int true=1, false=0
    ...
    cpdef Boundary get_shorty_boundary(self):
        ...
        l1 = b1._length # norm( b1.b[len(b1.b)-1] - b1.b[0] )
        l2 = b2._length # norm( b2.b[len(b2.b)-1] - b2.b[0] )
        l3 = b3._length # norm( b3.b[len(b3.b)-1] - b3.b[0] )
...
cdef class ThinFilm(object):
    ...
    def initialize_graph(self, num_grains=2500, num_interior_points_per_boundary=8,
            save_voronoi=None, load_voronoi=None):
        ...
        The Voronoi diagram used as seed can be saved or loaded from file
        by passing a filename to save_voronoi or load_voronoi.
        If both are passed, save_voronoi is ignored."""
        # magnification factor from Voronoi diagram to our simulation region:
        mag = np.sqrt(num_grains)
        self._domain_width = mag
        if load_voronoi is not None:
            with open(load_voronoi,'rb') as fobj:
                d = cPickle.load(fobj)
            segments = d['segments']
            nodelist = d['nodelist']
            node_segments = d['node_segments']
        else:
            segments, nodelist, node_segments = voronoi.periodic_diagram_with_nodes(num_grains)
            if save_voronoi is not None:
                d = dict()
                d['segments'] = segments
                d['nodelist'] = nodelist
                d['node_segments'] = node_segments
                with open(save_voronoi, 'wb') as fobj:
                    cPickle.dump(d,fobj)
    ...
    cpdef delete_single_boundary(self, Boundary gb):
        ...
        cdef:
            ...
            #np.ndarray[dtype=np.float64_t, ndim=1] head_node = np.empty((2))
            #np.ndarray[dtype=np.float64_t, ndim=1] tail_node = np.empty((2))
            np.ndarray[dtype=np.float64_t, ndim=1] t1 = np.empty((2))
            np.ndarray[dtype=np.float64_t, ndim=1] t2 = np.empty((2))
            np.ndarray[dtype=np.float64_t, ndim=1] t3 = np.empty((2))
            np.ndarray[dtype=np.float64_t, ndim=1] t4 = np.empty((2))
            np.ndarray[dtype=np.float64_t, ndim=1] p1 = np.empty((2))
            np.ndarray[dtype=np.float64_t, ndim=1] p2 = np.empty((2))
        ...
        if gb.head._type == domain_boundary:
            ...
            if gb.tail._type == triple_point:
                ...
                head_prev_indx = head_index-1 if head_index > 0 else len(gb.head.boundaries)-1
                head_prev_boundary = gb.head.boundaries[head_prev_indx]
                head_prev_end = gb.head.ends[head_prev_indx]
                ...
                head_subs_indx = head_index+1 if head_index+1 < len(gb.head.boundaries) else 0
                head_subs_boundary = gb.head.boundaries[head_subs_indx]
                head_subs_end = gb.head.ends[head_subs_indx]
                ...
                tail_prev_indx = tail_index-1 if tail_index > 0 else len(gb.tail.boundaries)-1
                tail_prev_boundary = gb.tail.boundaries[tail_prev_indx]
                tail_prev_end = gb.tail.ends[tail_prev_indx]
                ...
                tail_subs_indx = tail_index+1 if tail_index+1 < len(gb.tail.boundaries) else 0
                tail_subs_boundary = gb.tail.boundaries[tail_subs_indx]
                tail_subs_end = gb.tail.ends[tail_subs_indx]
                ...
                # This method ignores periodic b.c.:
                ...
                gb.tail.get_closest_interior_point(tail_prev_indx, t1)
                gb.head.get_closest_interior_point(head_subs_indx, t2)
                gb.head.get_closest_interior_point(head_prev_indx, t3)
                gb.tail.get_closest_interior_point(tail_subs_indx, t4)
                # Should instead just base this off the endpoints of the deleted boundary?
                
                for pt1, pt2 in [[t2,t1], [t3,t4]]:
                    if norm(pt2-pt1) > self._domain_width/2:
                        for i in [0,1]:
                            if pt2[i]-pt1[i] > self._domain_width/2:
                                pt2[i] -= self._domain_width
                            elif pt2[i]-pt1[i] < -self._domain_width/2:
                                pt2[i] += self._domain_width
                ...
                pairs     = [[tail_prev_boundary, head_subs_boundary],
                             [head_prev_boundary, tail_subs_boundary]]
                pair_ends = [[tail_prev_end, head_subs_end],
                             [head_prev_end, tail_subs_end]]
                pair_locs = [[t1, t2],
                             [t3, t4]]
                new_node_locs = [new_node_head, new_node_tail]
                for pair, ends, locs, new_node_loc in zip(pairs,pair_ends,pair_locs,new_node_locs):
                    for b, end, loc in zip(pair,ends,locs):
                        indx, iindx  = (0,1) if end == head_end else (len(b.b)-1, len(b.b)-2)
                        shift = b.b[iindx] - loc
                        b.b[indx] = new_node_loc + shift
                ...
                TP1 = Node(node_type=triple_point, period=self._domain_width)
                TP1.set_position(new_node_head)
                TP1.add_boundaries(
                    dict(b=tail_prev_boundary, end=tail_prev_end),
                    dict(b=head_subs_boundary, end=head_subs_end),
                    dict(b=new_gb, end=head_end))
                TP2 = Node(node_type=triple_point, period=self._domain_width)
                TP2.set_position(new_node_tail)
                TP2.add_boundaries(
    ...
    cdef int concatenate_boundaries(self, Boundary a, int aend, Boundary b, int bend) except *:
        ...
        cdef:
            ...
            np.ndarray[dtype=np.float64_t, ndim=1] a_node = np.empty((2))
            np.ndarray[dtype=np.float64_t, ndim=1] b_node = np.empty((2))
            np.ndarray[dtype=np.float64_t, ndim=1] shift  = np.empty((2))
        
        if verbose: print "Concatenating boundaries."
        ...
        if aend == tail_end:
            ...
            a.get_head_location(a_node)
            if bend == head_end:
                b_surviving_end = tail_end
                b.get_tail_location(b_node)
                shift = a_node - b_node
                new_array = np.concatenate((a.b[:len(a.b)-1], b.b[1:]+shift), axis=0)
            else:
                b_surviving_end = head_end
                b.get_head_location(b_node)
                shift = a_node - b_node
                new_array = np.concatenate((a.b[:len(a.b)-1], b.b[len(b.b)-1::-1]+shift), axis=0)
        else:
            a_surviving_end = tail_end
            a.get_tail_location(a_node)
            if bend == head_end:
                b_surviving_end = tail_end
                b.get_tail_location(b_node)
                shift = a_node - b_node
                new_array = np.concatenate((a.b[len(a.b)-1::-1], b.b[1:]+shift), axis=0)
            else:                    
                b_surviving_end = head_end
                b.get_head_location(b_node)
                shift = a_node - b_node
                new_array = np.concatenate((a.b[len(a.b)-1::-1], b.b[len(a.b)-1::-1]+shift), axis=0)
    ...
    def delete_several_boundaries(self, to_be_deleted):
        ...
        cdef Boundary bndry, sb, b1, b2, b3, b4, b_cw, b_ccw, survivor, b_opp
        cdef Node tp, node1, node2, new_node
        cdef np.ndarray[dtype=np.float64_t, ndim=1] loc   = np.empty((2))
        cdef np.ndarray[dtype=np.float64_t, ndim=1] shift = np.empty((2))
        cdef np.ndarray[dtype=np.float64_t, ndim=1] new_setpoint = np.empty((2))
        ...
        if len(to_be_deleted) == 3:
            ...
            if verbose: print "Deleting three-sided bubble."
            ...
            for i,tp in enumerate(ordered_nodes):
                survivor, survivor_end = tp.get_sole_survivor()
                tp.get_closest_interior_point(tp.boundaries.index(survivor), endpoints[i])
            ...
            for i,tp in enumerate(ordered_nodes):
                ...
                tp.get_closest_interior_point(tp.boundaries.index(survivor), loc)
                indx  = 0 if survivor_end == head_end else len(survivor.b)-1
                iindx = 1 if survivor_end == head_end else len(survivor.b)-2
                shift = survivor.b[iindx] - loc
                survivor.b[indx] = new_setpoint + shift
                if verbose:
                    print " Deleting bubble, shift boundary {0} by {1}".format(i,shift)
                    if any(survivor.b[:,0] > 0):
                        print "  boundary:", survivor.b
                self.nodes.remove(tp)

            self.boundaries.remove(b1)
            self.boundaries.remove(b2)
            self.boundaries.remove(b3)
            new_node = Node(node_type=triple_point, period=self._domain_width)
            new_node.set_position(new_setpoint)
            ...
            """ 5 followed by 4* """
            if verbose: print "Deleting four-sided bubble."
            b1, b2, b3, b4 = to_be_deleted
            ...
            if sb in [b2,b3,b4]:
                ...
                node1 = b1.tail
                node2 = b1.head
                sb, sb_endindx, sb_end = b1.get_subsequent_boundary(b1.tail)
            ...
            b_ccw, bccw_endindx, bccw_end = b1.get_previous_boundary(node2)
            if node2 == b_ccw.head:
                """ b_ccw also runs ccw head-> tail """
                node3 = b_ccw.tail
            else:
                node3 = b_ccw.head
            
            ordered_nodes = [node1, node2, node3, node4]
            endpoints = np.zeros((4,2), dtype=np.float64)
            for i,tp in enumerate(ordered_nodes):
                survivor, survivor_end = tp.get_sole_survivor()
                tp.get_closest_interior_point(tp.boundaries.index(survivor), endpoints[i])

            b_opp, bopp_endindx, bopp_end = b_cw.get_subsequent_boundary(node4)
            height = (b1._length + b_opp._length)/2
            width  = (b_cw._length + b_ccw._length)/2
            if width > height:
                # new boundary will be horizontal and run left->right (b1->b_opp)
                p1 = (endpoints[0]+endpoints[1])/2
                p2 = (endpoints[2]+endpoints[3])/2
                pair1 = [node1,node2]
                pair2 = [node3,node4]
            else:
                # new boundary will be vertical and run bottom->up (b_ccw -> b_cw)
                p1 = (endpoints[1]+endpoints[2])/2
                p2 = (endpoints[0]+endpoints[3])/2
                pair1 = [node2,node3]
                pair2 = [node1,node4]
            ...
            for pair, node_location, end in zip([pair1, pair2], [new_node_head, new_node_tail], [head_end, tail_end]):
                survivors     = []
                survivor_ends = []
                for i,tp in enumerate(pair):
                    survivor, survivor_end = tp.get_sole_survivor()
                    survivors.append(survivor)
                    survivor_ends.append(survivor_end)
                    tp.get_closest_interior_point(tp.boundaries.index(survivor), loc)
                    ...
                    iindx = 1 if survivor_end == head_end else len(survivor.b)-2
                    shift = survivor.b[iindx] - loc
                    survivor.b[indx] = node_location + shift
                    if verbose:
                        print " Deleting 4-sided bubble, shift boundary {0} by {1}".format(i,shift)
                        if any(survivor.b[:,0] > 0):
                            print "  boundary:", survivor.b
                    self.nodes.remove(tp)

                new_node = Node(node_type=triple_point, period=self._domain_width)
                new_node.set_position(node_location)
                new_node.add_boundaries(
                    dict(b=survivors[0], end=survivor_ends[0]),
                    dict(b=survivors[1], end=survivor_ends[1]),
                    dict(b=new_gb, end=end))
                self.nodes.append(new_node)
            
            for bndry in to_be_deleted:
                self.boundaries.remove(bndry)

        else:
            print "Wants to delete more than 4 boundaries at once. Fail."
            print "Boundaries:"
            for b in to_be_deleted:
                print b.get_boundary()
    ...
    def delete_boundaries(self, list to_be_deleted):
        ...
        cdef Boundary gb
        for gb in reversed(to_be_deleted):
        ...
        if len(to_be_deleted) > 2:
            ...
            deleted_groups = self.sort_deleted_boundaries_into_groups(to_be_deleted)
            for g in deleted_groups:
                if len(g) == 2:
                    self.delete_two_boundaries(g)
                else:
                    self.delete_several_boundaries(g)
                    
        elif len(to_be_deleted) == 2:
        ...
        elif len(to_be_deleted) == 1:
            """ this shouldn't happen because it should have been deleted above... """
            gb = to_be_deleted[0]
            self.delete_single_boundary(to_be_deleted[0])
        elif len(to_be_deleted) == 0:
            return True
            
                           
    def sort_deleted_boundaries_into_groups(self, list to_be_deleted):
        """ Group the boundaries into sets that will be deleted as a unit.
        Returns a list of lists of boundaries. """
        cdef Boundary gb, gb2
        if verbose: print "Sorting boundaries:", range(len(to_be_deleted))
        neighbors = []
        for gb in to_be_deleted:
            share_nodes = []
            for i,gb2 in enumerate(to_be_deleted):
                if gb2.has_node(gb.head) or gb2.has_node(gb.tail):
                    share_nodes.append(i)
            neighbors.append(share_nodes)
        
        groups = [[] for i in range(len(to_be_deleted))]
        for n in neighbors:
            for i in n:
                groups[min(n)].append(i)
        
        if verbose: print "Sorting start:", groups
        groups = [set(group) for group in groups]

        for group in reversed(groups):
            for member in group:
                groups[min(group)].add(member)
        ...
        extra_groups.sort()
        for i in reversed(extra_groups):
            groups.pop(i)

        if verbose: print "Sorted boundaries:", groups

        boundaries = []
        for g in groups:
            boundaries.append([to_be_deleted[i] for i in g])
            
        return boundaries        

    def get_node_locations(self):
        """ Return a list of all node positions """
        locs = []
        for n in self.nodes:
            locs.append(n.get_position().copy())
    ...
    def regrid(self):
        ...
        if len(to_be_deleted) > 0:
            if verbose: 
                print "Pre-deletion number of Nodes: {0}:".format(len(self.nodes))
            ...
            if verbose:
                print "    Post-deletion number of Nodes: {0}:".format(len(self.nodes))
    ...
    cpdef update_nodes(self):
        ...
        for tp in self.nodes:
            if -1 == tp.update_position():
                return self.reset_timestep()
    
    cpdef reset_timestep(self):
        """ Reset the boundaries and nodes to their values at the beginning
        of the current time-step. (Usually invoked after a re-gridding). """
        cdef Node     n
        cdef Boundary b
        for n in self.nodes:
            n.reset_timestep()
